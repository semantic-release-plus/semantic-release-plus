...
describe('get-config', () => {
  ...
  beforeEach(() => {
    jest.mock('./plugins', () => pluginsMock.mockReturnValue({}));
    jest.resetModules();
  });

  test.skip('Default values, reading repositoryUrl from package.json', async () => {
  ...
  test('Read options from package.json', async () => {
    ...
    expect(pluginsMock.mock.calls[0][0]).toEqual({ options: expected, cwd });
  });
  ...
  test('Read configuration from file path in "extends"', async () => {
    ...
    expect(pluginsMock.mock.calls[0][0]).toEqual({ options: expected, cwd });
    expect(pluginsMock.mock.calls[0][1]).toEqual({
  ...
  test('Allow to unset properties defined in shareable config with "null"', async () => {
    ...
    expect(pluginsMock.mock.calls[0][0]).toEqual({
      options: {
        ...omit(options1, 'analyzeCommits'),
        ...omit(pkgOptions, ['extends', 'analyzeCommits']),
        plugins: DEFAULT_PLUGINS,
  ...
  //   await t.throwsAsync(getConfig({ cwd }), {
